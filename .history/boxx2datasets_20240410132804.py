...
def genCSV():
    ...
    writeInCSV = []
    for Axml in xmlList:
        locations = readXML(Axml)
        with open(os.path.join('sets',Axml.split('.')[0]+'.txt')) as file:
...
if __name__ == '__main__':
    ...
    # 添加命令行参数
    parser.add_argument('type', help='第一步先生成txt,第二步再生成csv,因此第一次参数为txt,第二次参数为csv')
    ...
    if args.type == 'txt':
        ...
        raise ValueError('命令行参数输入错误查看help')
