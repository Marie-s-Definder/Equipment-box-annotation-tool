...
def genTxt():
    ...
    # print(xmlList)
    for Axml in xmlList:
        if os.path.exists(os.path.join('sets',Axml.split('.')[0]+'.txt')):
            print(f'文件{os.path.join('sets',Axml.split('.')[0]+'.txt')}存在，不会覆盖，请手动清除')
            continue
...
def genCSV():
    ...
    for Axml in xmlList:
        ...
        if len(locations) != 7:
            raise ValueError('标注时候拉框有问题，请检查是否拉了7个框，多一个少一个都不行')
